...
public class ThrillerAdapter extends RecyclerView.Adapter<ThrillerAdapter.ThrillerViewHolder> {

    Context context;

    ArrayList<ThrillerModel> ThrillerList;

    public ThrillerAdapter(Context context, ArrayList<ThrillerModel> thrillerList) {
    ...
    public ThrillerViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View thrillerView = LayoutInflater.from(context).inflate(R.layout.category_item, parent,false);
        return new ThrillerViewHolder(thrillerView);
    }

    @Override
    public void onBindViewHolder(@NonNull ThrillerViewHolder holder, int position) {

        ThrillerModel thrillerModel= ThrillerList.get(position);
        holder.item_name.setText(thrillerModel.getMovieName());
        String movieId= thrillerModel.getMovieId();
    ...
    public int getItemCount() {
        ...
        return ThrillerList.size();
    }

    public static class ThrillerViewHolder extends RecyclerView.ViewHolder{
